  ...
  const MultiSearch = () => {
    ...
    useEffect(() => {
      ...
      if (values?.search && values?.search.length > 0) {
        ...
        // Sanitize body: remove legacy min/max bedroom/bathroom fields and ensure arrays are present
        const sanitizedBody: any = { ...searchBody };
        delete sanitizedBody.min_bedrooms;
        delete sanitizedBody.max_bedrooms;
        delete sanitizedBody.min_bathrooms;
        delete sanitizedBody.max_bathrooms;
        sanitizedBody.selected_bedrooms = values?.selected_bedrooms || [];
        sanitizedBody.selected_bathrooms = values?.selected_bathrooms || [];

        console.log("[API BODY] Search suggestions body:", sanitizedBody);
        mutateAsync(sanitizedBody);
    ...
    // Log snapshot whenever selected arrays change
    useEffect(() => {
      const snapshot = {
        selected_bedrooms: values?.selected_bedrooms || [],
        selected_bathrooms: values?.selected_bathrooms || [],
      };
      console.log("[STATE SNAPSHOT] Beds/Baths:", snapshot);
    }, [values?.selected_bedrooms, values?.selected_bathrooms]);
        ...
        <Btn
          ...
          onclick={() => {
            ...
            setValues({
              ...
              price_min: undefined,
              price_max: undefined,
              // we now keep selected arrays only:
              selected_bedrooms: [],
              selected_bathrooms: [],
              area_min: undefined,
              area_max: undefined,
    ...
    const toggleBedroom = (val: number | "Studio" | "7+") => {
      ...
      const next = exists ? current.filter((v: any) => v !== val) : [...current, val];
      ...
      console.log("[STATE] selected_bedrooms updated:", next);
    };
    ...
    const toggleBathroom = (val: number | "7+") => {
      ...
      console.log("[STATE] selected_bathrooms updated:", next);
    };
    ...
    const onSubmit = () => {
      if (values?.type_name && values?.type_id) {
        const searchState = {
          ...
          // send selected arrays (no min/max)
          selected_bedrooms: values?.selected_bedrooms || [],
          selected_bathrooms: values?.selected_bathrooms || [],
          area: values?.area,
        ...
        console.log("Submitting search with values:", searchState);
        ...
        (values?.selected_bathrooms || []).forEach(
          (bath: number | string) => {
