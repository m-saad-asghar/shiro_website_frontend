...
const SingleDeveloperServices = {
  async fetchDeveloperBySlug(slug: string) {
    ...
    if (!response.ok) {
      throw new Error(`Failed to Fetch Developer, Status Code: ${response.status}`);
    }
...
const DeveloperDetails = () => {
  ...
  return (
    <>
      ...
      >
        {/* Hero Image */}
        {/* {developer?.developer_main_image && (
          <section className="w-full h-[92vh]">
            <img
              src={ImagesUrl(developer.developer_main_image)}
              alt={localizedName}
  ...
  {/* RIGHT 6 COLUMNS (ALIGN RIGHT) */}
  <div className="col-span-12 md:col-span-6 flex justify-end text-right">
    <div>
      {developer?.developer_main_image && (
